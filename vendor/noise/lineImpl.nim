...
proc calcLineEndPos(self: Line, upTo: int, promptWidth, screenWidth: int): tuple[x, y: int] =
  # Multi-line aware position calculator: handles explicit '\n' in buffer
  var col = promptWidth
  var row = 0
  for i in 0 ..< upTo:
    let c = self.data[i]
    if c == '\n'.ord:
      ...
      let w = mk_wcwidth(c.int32)
      let charW = if w > 0: w else: 1
      inc(col, charW)
...
proc refreshLine(self: var Line, prompt: var Prompt) =
  ...
  let endOfInput = self.calcLineEndPos(self.dataLen, promptWidth, screenWidth)
  let cursorPos = self.calcLineEndPos(self.pos, promptWidth, screenWidth)
  ...
  when not defined(windows):
    # generate our own newline on line wrap, but not if content ends with an
    # explicit newline (which already moved the cursor to the next line)
    if endOfInput.x == 0 and endOfInput.y > 0:
      if self.dataLen == 0 or self.data[self.dataLen - 1] != '\n'.char32:
        stdout.write "\n"
