...
proc wtf8_decode*[T](str: openArray[T], maxBytes: int): tuple[codePoint, status: int] =
  var state = 0
  result.codePoint = 0xFFFD

  for i in 0..<maxBytes:
    result.status = wtf8_decode_state(state, result.codePoint, str[i].int)
    if result.status == UTF8_REJECT: return
...
template encodeUTF16(codepoint, output: typed) =
  if codepoint > 0xffff:
    #make a surrogate pair
    let cp = codepoint - 0x10000
    output.add(uint16((cp shr 10) + LEAD_OFFSET))
    output.add(uint16((cp and 0x3ff) + TRAIL_SURROGATE_MIN))
  else:
    output.add(uint16(codepoint))
...
proc utf32to16*(se: openArray[int32], limit: int): seq[uint16] =
  result = @[]
  var pos = 0
  while pos < limit and pos < se.len:
    encodeUTF16(se[pos].int, result)
    inc pos

proc is_lead_surrogate*(cp: int): bool {.inline.} = cp >= LEAD_SURROGATE_MIN and cp <= LEAD_SURROGATE_MAX
proc surrogatePair*(lead: int, trail: int): int {.inline.} =
  result = (lead shl 10) + trail + SURROGATE_OFFSET
...
proc utf8to32*(str: string): seq[int32] =
  result = @[]
  for codepoint in wtf8_decode(str):
    result.add(int32(codepoint))

proc utf8to32*(str: string, output: var seq[int32]): int =
  var i = 0
  for codepoint in wtf8_decode(str):
    output[i] = int32(codepoint)
    inc i
    if i == output.len: break
  result = i

proc utf8to32*(str: string, output: var seq[int32], pos, len: int): int =
  var i = 0
  for codepoint in wtf8_decode(str):
    output[i + pos] = int32(codepoint)
    inc i
    if i == len: break
  result = i
...
proc utf32to8*(se: openArray[int32], limit: int): string =
  result = ""
  var pos = 0
  while pos < limit and pos < se.len:
    wtf8_encode(se[pos].int, result)
    inc pos

proc replaceInvalid*(src: string): string =
  result = ""
