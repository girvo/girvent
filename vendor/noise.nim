...
#
#  NOTE: This includes some changes by me
  ...
  proc readLine*(self: var Noise): bool =
    ...
    if isAtty(stdin):
      if self.unsupportedTerm:
        ...
        stdout.write("\e[>1u")      # kitty keyboard protocol: disambiguate (sends ESC[13;2u for Shift+Enter)
        stdout.write("\e[>4;2m")    # xterm modifyOtherKeys=2 (sends ESC[27;2;13~ for Shift+Enter)
        stdout.flushFile()
        result = self.getInputLine()
        stdout.write("\e[>4;0m")    # restore modifyOtherKeys
        stdout.write("\e[<u")       # restore kitty keyboard protocol
